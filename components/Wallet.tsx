...
const Wallet: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      // Seed initial data
      const initialCash = 9500000;
      const initialTransactions: WalletTransaction[] = [
        { time: new Date(Date.now() - 7200000).toISOString(), type: 'DEPOSIT', note: 'Khởi tạo demo', amount: 3500000, balanceAfter: 3500000 },
        { time: new Date(Date.now() - 2100000).toISOString(), type: 'DEPOSIT', note: 'Nạp tiền mô phỏng', amount: 5000000, balanceAfter: 8500000 },
        { time: new Date(Date.now() - 20000).toISOString(), type: 'DEPOSIT', note: 'Nạp tiền mô phỏng', amount: 1000000, balanceAfter: 9500000 }
      ];
      setCash(initialCash);
      setTotalDeposit(initialCash);
      setTransactions(initialTransactions);
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ cash: initialCash, totalDeposit: initialCash, transactions: initialTransactions }));
    }
  }, []);
